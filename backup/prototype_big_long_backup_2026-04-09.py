...
# 🚀 [Big Long 修改] 更改系統名稱
logger = logging.getLogger('AlgoTrade_Big_Long_V6.0')

# ⚠️ API 金鑰配置 (請確保安全)
# 🚀 [Big Long 修改] 假設此處與 config.yaml 內 API 區塊一致
# Name: yamato
...
# 🚀 [Big Long 修改] 更改 Log 檔案名稱為 big_long
...
# ==========================================
# ⚙️ [系統/參數] 策略與風控全局變數 (對齊 config.yaml)
# ==========================================
# --- 基礎資金管理 ---
...
# POSITION_CHECK_INTERVAL = 10 (後改為 12)
# MIN_IMBALANCE_RATIO = 0.2
# PROFIT_LOCK_THRESHOLD = 0.015, PROFIT_RETRACE_LIMIT = 0.4
...
def process_native_exit_log(symbol, pos, position_type='long'):
    ...
    try:
        ...
        try:
            ...
            # 🚀 [Big Long 修改] 備用 PnL 計算 (Long: 現價 - 入場價)
...
def get_btc_regime():
    ...
    try:
        ...
        # ❌ [Short 原版保留] 條件 1：HMA20 跌穿 HMA50 (無滯後跌勢確立)
        # cond_trend = hma20_val < hma50_val
        # 🚀 [Big Long 修改] 條件 1：HMA20 升穿 HMA50 (無滯後升勢確立)
        cond_trend = hma20_val > hma50_val
...
# ❌ [Short 原版保留]
# def scouting_weak_coins(n=20):
#     """動態大幣海選：於全市場 Top 20 流動性巨無霸中，尋找最弱勢幣種"""
# 🚀 [Big Long 修改] 函數名改為 scouting_strong_coins，尋找最強勢幣種
def scouting_strong_coins(n=20):
    ...
    try:
        ...
        # 按 24 小時成交額強制抽取 Top 20，確保平倉時具備充足流動性
        top_20_majors = df.sort_values('volume', ascending=False).head(20)

        # ❌ [Short 原版保留] 於 Top 20 大幣中，選出跌幅最大 (最弱勢) 的 n 隻幣
        # return top_20_majors.sort_values('change', ascending=True).head(n)['symbol'].tolist()

        # 🚀 [Big Long 修改] 於 Top 20 大幣中，選出漲幅最大 (最強勢) 的 n 隻幣
        return top_20_majors.sort_values('change', ascending=False).head(n)['symbol'].tolist()
...
def sync_positions_on_startup():
    ...
    try:
        ...
        for p in live_symbols:
            ...
            if p.get('side') == 'buy' or float(p.get('positionValue', 0)) > 0 or (
                    p.get('info') and p['info'].get('side') == 'Buy'):
                ...
                recovered_count += 1
                # ❌ [Short 原版保留] print(f"✅ 成功尋回孤兒空單: {symbol} | 入場價: {entry_price}")
                # 🚀 [Big Long 修改]
                print(f"✅ 成功尋回孤兒多單: {symbol} | 入場價: {entry_price}")
...
#     """管理在途空單 (Native Exit 檢查、Trail SL 更新、回撤鎖利)"""
# 🚀 [Big Long 修改] 管理多單
        ...
        for s in list(positions.keys()):
            ...
            if pos['max_pnl_pct'] > PROFIT_LOCK_THRESHOLD:
                if pnl_pct < (pos['max_pnl_pct'] * (1 - PROFIT_RETRACE_LIMIT)):
                    # ❌ [Short 原版保留] exit_reason = "Profit Retrace Lock (Short IOC Exit)"
                    # 🚀 [Big Long 修改]
            ...
            if exit_reason:
                ...
                try:
                    ...
                    # exchange.create_order(s, 'limit', 'buy', pos['amount'], ioc_price, {'timeInForce': 'IOC', 'reduceOnly': True})

                    # 🚀 [Big Long 修改] 多單平倉要 Sell，所以參考買盤 bids 以減少滑價
                    ioc_price = get_3_layer_avg_price(s, 'bids') or curr_p
                    exchange.create_order(s, 'limit', 'sell', pos['amount'], ioc_price,
                                          {'timeInForce': 'IOC', 'reduceOnly': True})
                except:
                    # ❌ [Short 原版保留] exchange.create_market_buy_order(s, pos['amount'], {'reduceOnly': True})
                    # 🚀 [Big Long 修改] 多單市價平倉
                    exchange.create_market_sell_order(s, pos['amount'], {'reduceOnly': True})

                # ❌ [Short 原版保留] ioc_pnl = round((pos['entry_price'] - curr_p) * pos['amount'], 4)
                # 🚀 [Big Long 修改]
...
def execute_live_long(symbol, net_flow, current_price, is_strong, atr, is_volatile):
    ...
    if symbol in cooldown_tracker:
        ...
        # ❌ [Short 原版保留] if not (is_weak and is_volatile and symbol not in positions): return
    # 🚀 [Big Long 修改] 確保是強勢
    ...
    # ❌ [Short 原版保留] ioc_p = get_3_layer_avg_price(symbol, 'bids') or current_price
    # 🚀 [Big Long 修改] 多單入場要 Buy，所以參考賣盤 asks 以減少滑價
    ...
    try:
        ...
        if expected_profit_margin < 0.003:
            # ❌ [Short 原版保留] print(f"🟡 放棄做空 [{symbol}]: 預期利潤空間 ({expected_profit_margin * 100:.2f}%) 太細！")
            # 🚀 [Big Long 修改]
            print(f"🟡 放棄做多 [{symbol}]: 預期利潤空間 ({expected_profit_margin * 100:.2f}%) 太細！")
            cancel_all_v5(symbol)
            return
        ...
        # ❌ [Short 原版保留] print(f"📉 [已入貨做空] {symbol} @ {actual_price:.4f} | 數量: {actual_amount}")
        # 🚀 [Big Long 修改]
        print(f"📈 [已入貨做多] {symbol} @ {actual_price:.4f} | 數量: {actual_amount}")

    except Exception as e:
        # ❌ [Short 原版保留] logger.error(f"❌ {symbol} 做空核心執行失敗: {e}")
        # 🚀 [Big Long 修改]
...
def main():
    ...
    while True:
        try:
            # 1. 倉位管理與本地防禦
            # ❌ [Short 原版保留] manage_short_positions()
            # 🚀 [Big Long 修改]
            ...
            if curr_t - last_scout_time > SCOUTING_INTERVAL:
                ...
                if regime == 1:
                    # ❌ [Short 原版保留] print("🟢 綠燈確認：執行空單大幣海選掃描...")
                    # 🚀 [Big Long 修改]
                    print("🟢 綠燈確認：執行多單大幣海選掃描...")

                    # ❌ [Short 原版保留] target_coins = scouting_weak_coins(20)
                    # 🚀 [Big Long 修改] 選出強勢幣
                ...
                # ❌ [Short 原版保留] print(f"⏳ 空軍巡邏完畢 | 持倉: {list(positions.keys())} | 餘額: {get_live_usdt_balance():.2f}")
                # 🚀 [Big Long 修改]

...
TP_ATR_MULT = 5.0                                       # 止盈 ATR 倍數 🚀 放闊止盈，讓暴漲利潤奔跑
SL_ATR_MULT = 1.5                                       # 初始止損 ATR 倍數
MIN_IMBALANCE_RATIO = 0.15                              # 訂單簿失衡度門檻 (買盤需厚於賣盤)
...
def check_flow_health(symbol):
    """【防守專用 - 多單版】資金流健康雷達：砸盤(Reversal)與升勢衰退(Deceleration)檢測"""
    try:
        ...
        # 🚨 防護 1：極端砸盤 (Panic Sell)
        if z_score < -3.0:
            return "Flow Reversal (Panic Sell Detected)"

        # 🚀 防護 2：升勢衰退 (Deceleration) - 提早收油止盈
        ...
        # 多軍煞車：加速度極強向下 (大戶急停) 且 資金流轉負 (開始出貨)
...
def apply_lee_ready_long_logic(symbol):
    ...
    try:
        ...
        if not trades: return 0, 0, False, False, 0, 0
        ...
        if std_val > 0:
            z_score = short_window_flow / (std_val * np.sqrt(50))
        else:
            z_score = 0
    ...
    try:
        ...
        for p in live_symbols:
            ...
            if side in ['long', 'buy'] or info_side in ['buy', 'long']:
                ...
                # 判斷多單 SL 係咪已經高過入場價 (且大於0)，防止 Trail SL 倒退
        ...
        for s, p in live_symbols.items():
            if s not in positions:
                ...
                if side in ['long', 'buy'] or info_side in ['buy', 'long']:
                    ...
                    print(f"🚨 [系統自癒] 發現並自動接管孤兒多單: {s} | 入場價: {entry_p} | 已保本狀態: {is_be}")
        # ==========================================
        ...
        for s in list(positions.keys()):
            ...
            if pos['is_breakeven']:
                if pnl_pct > (coin_volatility_pct * 3.5):
                    trail_sl = curr_p - (1.5 * pos['atr'])  # 暴漲期收緊風箏線
                else:
                    trail_sl = curr_p - (2.0 * pos['atr'])  # 正常爬升期放鬆風箏線
            ...
            # ==========================================
            # 🚀 常規本地 TP/SL 檢查 (做多版本：升穿TP止盈，跌穿SL止損)
            # ==========================================
            if not exit_reason:
                if curr_p >= pos['tp_price']:  # ✅ 升到或升穿止盈價 (>=)
                    exit_reason = "TP (Long IOC Exit)"
                elif curr_p <= pos['sl_price']:  # ✅ 跌到或跌穿止損價 (<=)
...
def execute_live_long(symbol, net_flow, current_price, is_strong, atr, is_volatile, flow_is_fresh, regime_info):
    ...
    try:
        ...
        cooldown_tracker[symbol] = time.time() + 480  # 8 分鐘 (480秒) 冷卻
...
# ==========================================
def main():
    print(f"🚀 AI 實戰 V6.0 FINAL LONG (重裝甲防護版) 啟動...")
        ...
        try:
            ...
            if curr_t - last_scout_time > SCOUTING_INTERVAL:
                ...
                if regime_info['signal'] == 1:
                    ...
                    print(f"🚦 目前導航狀態為 Signal: {regime_info['signal']}，海選暫停。")
